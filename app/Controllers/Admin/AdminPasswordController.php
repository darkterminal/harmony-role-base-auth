...
class AdminPasswordController extends Controller
{
    ...
    {
        return $this->view->render($response, 'admin/password/forgot-password.twig');
    }
    ...
    public function postForgotPassword($request, $response)
    {
        ...
        $emailExists = Admins::where('email', $email)->first();

        if (empty($emailExists)) {
            $this->flash->addMessage('error', 'Your Email does\'t exists');
            return $response->withRedirect($this->router->pathFor('admin.forgot.password'));
        }

        $user = Admins::findOrFail($emailExists->id);
    ...
    {
        $user = Admins::where('email', $request->getParam('email'))->first();
        if ($user->password_reset == urlencode($request->getParam('token'))) {
            
            return $this->view->render($response, 'admin/password/reset-password.twig', ['email' => $request->getParam('email')]);
